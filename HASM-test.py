...
x= [38, 28, 14, 42,  7, 20, 38, 18, 22, 10, 10, 23, 35, 39, 23]
y= [ 2, 21,  1, 23, 43, 29, 37,  1, 20, 32, 11, 21, 43, 24, 26]
nombre=len(x)

for i in range(nombre):
    fonc_init[x[i]*(J-2)+y[i],0]=coro(x[i]+1, y[i]+1)
    fo[(x[i]+1)*J+(y[i]+1),0] =coro(x[i]+1, y[i]+1) 
...
Sampling = np.zeros((n, n))
for i in range(nombre):
    Sampling[x[i]*(J-2)+y[i],x[i]*(J-2)+y[i]]=1
...
RMSE_rela=0
for i in range(nombre):
        RMSE_rela=RMSE_rela+(Z[x[i]+1, y[i]+1]-Solu[x[i],y[i]])**2
