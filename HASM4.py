# -*- coding: utf-8 -*-
"""
Created on Tue May 21 11:51:00 2024
...
x= random.sample(range(0,I-2),10)
y= random.sample(range(0,J-2),10)
for i in x:
    for j in y:
...
while incre != 1:
    ...
    fina = np.reshape(fonc_init, ( I1 , J1 ))
    ...
    for i in range(I1):
        for j in range(J1):
            critere=critere+(Solu[i,j]-fina[i,j])**2
    critere=math.sqrt(critere/I1*J1)    
...
for i in x:
    for j in y:
        ...
        MAE_rela=MAE_rela+np.abs(Z[i+1,j+1]-Solu[i,j])
RMSE_rela=math.sqrt(RMSE_rela/(len(x)*len(y)))
MAE_rela=(MAE_rela/(len(x)*len(y)))
print("RMSE_rela= ", RMSE_rela," MAE_rela=", MAE_rela)
