"""
Created on Sat Jul 20 15:00:42 2024
...
def coro(i,j):
    retour=i*(J)+j
    if retour<0 or retour>I*J-1 or i<0 :
        retour=0
    else :
...
x= random.sample(range(0,I-2),4)
y= random.sample(range(0,J-2),4)
for i in x:
    for j in y:
...
def E(i,j):
    retour=1 + ((cor(i+1, j) - cor(i-1, j)) / (2 * h))**2
    return retour
def G(i,j):
    retour=1 + ((cor(i, j+1) - cor(i, j-1)) / (2 * h))**2
    return retour
def F(i,j):
    retour=((cor(i+1, j) - cor(i-1, j)) / (2 * h))*((cor(i, j+1) - cor(i, j-1)) / (2 * h))
    return retour
def L(i,j):
    retour=((cor(i+1, j) - 2*cor(i, j) +  cor(i-1, j))/h**2)/math.sqrt(1+ ((cor(i+1, j) - cor(i-1, j)) / (2 * h))**2 + ((cor(i, j+1) - cor(i, j-1)) / (2 * h))**2 )
    return retour
def N(i,j):
    retour=((cor(i, j+1)- 2*cor(i, j) +  cor(i, j-1))/h**2)/math.sqrt(1+ ((cor(i+1, j) - cor(i-1, j)) / (2 * h))**2 + ((cor(i, j+1) - cor(i, j-1)) / (2 * h))**2 )
    return retour
def M(i,j):
    retour=(( ((cor(i+1, j+1) - cor(i+1, j-1))) / 4* h**2) -( ((cor(i-1, j+1) - cor(i-1, j-1))) / 4* h**2)  )/math.sqrt(1+ (( cor(i+1, j) - cor(i-1, j)) / (2 * h))**2 + ((cor(i, j+1) - cor(i, j-1)) / (2 * h))**2 )
    return retour
...
#         E[i+1,j+1] = 1 + ((cor(i+1, j) - cor(i-1, j)) / (2 * h))**2
#         G[i+1,j+1] = 1 + ((cor(i, j+1) - cor(i, j-1)) / (2 * h))**2
#         F[i+1,j+1] = ((cor(i+1, j) - cor(i-1, j)) / (2 * h))*((cor(i, j+1) - cor(i, j-1)) / (2 * h))
#         L[i+1,j+1] =((cor(i+1, j) - 2*cor(i, j) +  cor(i-1, j))/h**2)/math.sqrt(1+ ((cor(i+1, j) - cor(i-1, j)) / (2 * h))**2 + ((cor(i, j+1) - cor(i, j-1)) / (2 * h))**2 )
#         N[i+1,j+1] =((cor(i, j+1)- 2*cor(i, j) +  cor(i, j-1))/h**2)/math.sqrt(1+ ((cor(i+1, j) - cor(i-1, j)) / (2 * h))**2 + ((cor(i, j+1) - cor(i, j-1)) / (2 * h))**2 )
#         M[i+1,j+1] =(( ((cor(i+1, j+1) - cor(i+1, j-1))) / 4* h**2) -( ((cor(i-1, j+1) - cor(i-1, j-1))) / 4* h**2)  )/math.sqrt(1+ (( cor(i+1, j) - cor(i-1, j)) / (2 * h))**2 + ((cor(i, j+1) - cor(i, j-1)) / (2 * h))**2 )
...
for i in range(1,I-1):
    ...
    if (i+1)==(I-1) :
        a6=1
    for j in range(1,J-1):
        valeur[j-1,0]=(1/2)*(cor(i+1, j) - cor(i-1, j))*G111[i,j]*h + (1/2)*(cor(i, j+1)-cor(i, j-1))*G211[i,j]*h + L(i,j)*h**2/math.sqrt(E(i,j)+G(i,j)-1) - a5*cor(0, j) - a6*cor(1, j)
...
for i in range(1,I-1):
    ...
    for j in range(1,J-1):
        ...
        if (j+1)==(J-1) :
            a6=1
        valeur[j-1,0]=(1/2)*(cor(i+1, j)-cor(i-1, j))*G122[i,j]*h + (1/2)*(cor(i, j+1)-cor(i, j-1))*G222[i,j]*h + N(i,j)*h**2/math.sqrt(E(i,j)+G(i,j)-1) - a5*cor(i, 0) - a6*cor(i, J-1)
